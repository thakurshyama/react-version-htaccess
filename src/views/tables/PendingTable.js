...
function GetApi() {
  ...
  const fetchUser = () => {
    axios.get(`http://139.59.60.119:4777/v1/order/outlets?status=pending&from=01-19-2023&to=${d.toDateString()}`, {
  ...
  const [isModalOpenpen, setIsModalOpenpen] = useState(false);
  const showModalpen = () => {
    setIsModalOpenpen(true);
  };
  const handleOkpen = () => {
    setIsModalOpenpen(false);
  };

  const [isModalOpenpen2, setIsModalOpenpen2] = useState(false);
  const showModalpen2 = () => {
    setIsModalOpenpen2(true);
  };
  const handleOkpen2 = () => {
    setIsModalOpenpen2(false);
  };

  const [isModalOpenpen3, setIsModalOpenpen3] = useState(false);
  const showModalpen3 = () => {
    setIsModalOpenpen3(true);
  };
  const handleOkpen3 = () => {
    setIsModalOpenpen3(false);
  };
            ...
            return (
              <tr key={user.orderId}>
                <td className='userinfo'>
                  <p type="primary" onClick={showModalpen}>
                    {user.client.clientName}
                  </p>
                  <Modal title="UserInformation" open={isModalOpenpen} onOk={handleOkpen}>
                ...
                <td>{user.status}</td>
                <td className='userinfo'>
                  <p type="primary" onClick={showModalpen2}>
                  {user.outlet.outletId}
                  </p>
                  <Modal title="UserInformation" open={isModalOpenpen2} onOk={handleOkpen2}>
                ...
                <td className='userinfo'>
                  <p type="primary" onClick={showModalpen3}>
                  {user.amount.totalAmount}
                  </p>
                  <Modal title="UserInformation" open={isModalOpenpen3} onOk={handleOkpen3}>
